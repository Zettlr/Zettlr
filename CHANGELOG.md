...
- Replaced the `make.sh` script with a `Makefile`. Refer to `make help`, but standard workflow would be `make build`, then `make release-all` or `make release-<platform>`.
