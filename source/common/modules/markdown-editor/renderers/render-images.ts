...
class ImageWidget extends WidgetType {
  constructor (readonly imageTitle: string, readonly imageUrl: string, readonly altText: string, readonly data: string) {
    super()
  }
  ...
  toDOM (view: EditorView): HTMLElement {
    ...
    let isDataUrl = /^data:[a-zA-Z0-9/;=]+(?:;base64){0,1},.+/.test(this.imageUrl)
    let actualURLToLoad = this.imageUrl
    ...
    img.onerror = () => {
      img.src = img404
      isDataUrl = true
      caption.textContent = 'Image not found' // TODO: Translate
    }
...
function createWidget (state: EditorState, node: SyntaxNodeRef): ImageWidget|undefined {
  ...
  return new ImageWidget(title, url, altText, p4)
}
