...
class VimFixedKeyboardPlugin implements PluginValue {
  ...
  destroy (): void {
    ...
    console.log('[Vim Fixed Keyboard CM6] Plugin destroyed')
  }
  ...
  private handleKeydown (event: KeyboardEvent): void {
    ...
    const cm = (Vim as any).getCM?.(this.view)
    ...
    console.log(`[Vim Fixed Keyboard CM6] Key pressed - code: ${event.code}, key: ${event.key}, shift: ${event.shiftKey}, mode: ${mode}, vimCommand: ${vimCommand}`)
    ...
    if (vimCommand !== null) {
      ...
      try {
        ...
        console.error('[Vim Fixed Keyboard CM6] Error handling vim command:', error)
      } finally {
