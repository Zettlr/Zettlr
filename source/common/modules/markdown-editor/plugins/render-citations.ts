...
interface Citation {
  ...
  textMarker: CodeMirror.TextMarker<CodeMirror.MarkerRange>
}
/**
   ...
   * @var {Object[]}
   */
let toRender: Citation[] = []

const commands = (CodeMirror.commands as any)
commands.markdownRenderCitations = async function (cm: CodeMirror.Editor) {
  let match
  ...
  for (let i = viewport.from; i < viewport.to; i++) {
    ...
    while ((match = citationRE.exec(line)) != null) {
      ...
      if (cm.getDoc().findMarks(curFrom, curTo).length > 0) continue
