...
export default class TutorialOpen extends ZettlrCommand {
  ...
  async run (evt: string, arg: any): Promise<any> {
    ...
    const { exact, close } = findLangCandidates(global.config.get('appLang'), candidates)
    ...
    try {
      ...
      global.log.info(`The directory ${targetPath} already exists.`)
    } catch (err) {
      ...
      global.log.info('Successfully copied the tutorial files', tutorialFiles)
    }
