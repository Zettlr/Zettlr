...
export default async function environmentCheck (logger: LogProvider): Promise<void> {
  ...
  if (!winARM && !macARM && !is64Bit && !isLinux && !linuxARM) {
    ...
    logger.warning(`[Application] Your platform/arch (${process.platform}/${process.arch}) combination is not officially supported. Zettlr might not function correctly.`)
  }
  ...
  if (isFile(pandocPath)) {
    logger.info(`[Application] Pandoc has been bundled with this release. Path: ${pandocPath}`)
    ...
    if (isFile(resPath)) {
      logger.info(`[Application] App is unpackaged, and Pandoc has been found in the resources directory: ${resPath}`)
      process.env.PANDOC_PATH = resPath
    } else {
      logger.warning(`[Application] App is unpackaged, but there was no Pandoc executable: ${resPath}`)
    }
  } else {
    logger.warning('[Application] Pandoc has not been bundled with this release. Falling back to system version instead.')
  }
  ...
  for (let p of REQUIRED_DIRECTORIES) {
    try {
      ...
      logger.info(`Creating required directory ${p} ...`)
