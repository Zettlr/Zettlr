      ...
      <div id="file-tree" ref="directories" v-on:click="selectionListener">
        ...
        <div id="file-list" class="hidden" ref="fileList" tabindex="1" v-on:keydown="navigate">
          ...
          <template v-else-if="getDirectoryContents.length > 1">
            ...
            <virtual-list
              ref="virtualFileList"
              v-bind:size="getFileListItemHeight"
              v-bind:remain="getVisibleElementCount"
              v-bind:item="listComponent"
              v-bind:itemcount="getDirectoryContents.length"
              v-bind:itemprops="getItem"></virtual-list>
          </template>
          <template v-else-if="getDirectoryContents.length === 1">
            <file-item
...
module.exports = {
  ...
  watch: {
    ...
    getDirectoryContents: function () {
      ...
      // We need to update the list after each directory update
      if (this.$refs.virtualFileList) this.$refs.virtualFileList.forceRender()
    },
    getFileListItemHeight: function () {
      if (this.$refs.virtualFileList) this.$refs.virtualFileList.forceRender()
    },
    getVisibleElementCount: function () {
      if (this.$refs.virtualFileList) this.$refs.virtualFileList.forceRender()
    },
    /**
  ...
  computed: {
    ...
    getVisibleElementCount: function () {
      // This function should return a number of elements that are visible, which
      // means: We have to first retrieve the overall height of the sidebar,
      // calculate the number of elements that it can fit, and return that, or
      // if the overall number of elements is smaller, that one.
      let sidebarHeight = document.getElementById('sidebar').clientHeight
      let itemHeight = this.getFileListItemHeight
      let elementsVisible = Math.round(sidebarHeight / itemHeight)
      console.log(`${elementsVisible} elements are visible within ${sidebarHeight}px`)
      return (elementsVisible > this.getDirectoryContents.length) ? this.getDirectoryContents.length : elementsVisible
    },
    getFileListItemHeight: function () {
      // Okay, now it gets funky. Because the virtual scroller requires
      // an exact height, and we have to give it something so that the
      // file items are squeezed a little bit in order to fit the full
      // size and no random problems emerge such as "holes" in the list.
      let sidebarHeight = document.getElementById('sidebar').clientHeight
      let itemHeight = (this.$store.state.fileMeta) ? 61 : 31 // Initial height
      let elementsVisible = Math.ceil(sidebarHeight / itemHeight) - 1
      // Now we have the amount of elements visible with (probably)
      // a little bit of spacing, so we need to see how much and divide
      // that spacing so that ALL elements in viewport are a little bit
      // bigger.
      let spacing = sidebarHeight - elementsVisible * itemHeight
      if (spacing > 0) itemHeight += spacing / elementsVisible
      console.log(`Calculated height (item): ${itemHeight}px`)
      return itemHeight
    }
  },
  methods: {
    ...
    /**
     * Called by the vue-virtual-list to retrieve
     * the correct data object to bind to the
     * element.
     */
    getItem: function (index) {
      let ret = {}
      Object.assign(ret, this.getDirectoryContents[index])
      return {
        key: index,
        props: ret // Vue will map everything in the "props" attribute onto the $props in the node
      }
    },
    /**
